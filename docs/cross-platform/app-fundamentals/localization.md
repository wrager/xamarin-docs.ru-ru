...
[ ![](localization-images/language-compare-sml.png "Длина японский строки немецком vs")](localization-images/language-compare.png)
...
• «On» — отображается на коммутаторе сам • «Off» – отображается на коммутаторе сам • «On» – отображается в • метку «Off» – отображается в метке

Это обеспечивает максимальную гибкость транслятора:

• В целях разработки может быть коммутатора использует строчные «on» и «off», но отображать подпись использует верхний регистр «On» и «Off».
...
• Кроме того, для некоторых языков, возможно, для подготовки к просмотру коммутатора использовать «I» и «O» для культуры вы знакомы, но вы можете по-прежнему метки для чтения «On» или «Off».
...
Для тестирования, возможно, это позволяет использовать один из многих средств Онлайн-перевод для включения некоторых локализованный текст в приложении во время разработки.

- [Переводчик Bing](https://www.bing.com/translator/) <!--Microsoft's Multilingual Application Toolkit helps you automatically translate strings, and is demonstrated with Xamarin.Forms in [this sample]().-->
