...
description: В этом документе описывается замените обращается Ваш компонент пакеты NuGet, чтобы в будущем приложений, так как хранилище компонентов Xamarin больше не поддерживаются.
