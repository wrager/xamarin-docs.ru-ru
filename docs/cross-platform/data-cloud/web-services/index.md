...
> [!IMPORTANT]
> **Примечание для Xamarin.iOS:** в iOS 9 безопасности транспорта приложения (ATS) обеспечивает безопасное соединение между Интернет-ресурсов (например, приложение серверных) и приложения, чтобы предотвратить случайное раскрытие конфиденциальных сведений. Поскольку ATS включена по умолчанию в приложениях, разработанных для iOS 9, все соединения будут применяться ATS требования безопасности. Если соединения не удовлетворяют этим требованиям, произойдет сбой с исключением.
...
[ ![](images/add-webreference-dialog.png "URL веб-службы может быть размещенного удаленного источника или через префикс пути файла ресурса локальной файловой системы")](images/add-webreference-dialog.png)
