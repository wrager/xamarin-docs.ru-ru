...
Каждый интерфейс в определении контракта, который имеет `[BaseType]` атрибут, объявляющий базовый тип для создаваемого объекта. В приведенном выше объявлении `MyType` будет создан класс тип C#, привязывается к типу Objective-C именем **MyType**.
...
В приведенном выше примере вы увидите, что мы решили перезаписать два метода, одно уведомление, что прокрутки событие имело место и второй, обратный вызов, который должен возвращать значение типа boolean, предписывая scrollView ли его следует перейти к верхней или не.
...
Он возвращает логическое значение, в этом случае синтаксис лямбда-выражения позволяет нам достаточно вернуть значение из `MakeDecision` функции.

Генератор привязки поддерживает создание события и свойства, связывающие класса, например `UIScrollView` с его `UIScrollViewDelegate` (также вызвать эти класс модели), это делается путем создания заметок к `BaseType` определение с `Events` и `Delegates`параметры (как описано ниже). Помимо добавления заметок к `BaseType` с этими параметрами это необходимо для информирования генератор несколько дополнительных компонентов.

Для событий, которые принимают несколько параметров (в Objective-C соглашением является первый параметр в класс делегата экземпляр объекта-отправителя) необходимо указать имя для создаваемого класса EventArgs может иметь. Это делается с `EventArgs` атрибутом в объявлении метода в классе модели. Пример:
...
Затем он предоставляет следующие параметры в класс UIImagePickerController:
...
Эти свойства всегда устанавливаются вместе и должны иметь одинаковое количество элементов и обеспечивать синхронизацию. `Delegates` Массив содержит одну строку для каждого слабо типизированной делегат, который требуется перенести, и события массив содержит один тип для каждого типа, который требуется связать с ним.
...
Чтобы привязать их, можно использовать `[Category]` атрибут в определении интерфейса.   При использовании `Category` атрибута значение `[BaseType]` изменений неизменных атрибутов не могут использоваться для указания базового класса для расширения, должен иметь тип для расширения.
...
`Abstract` Атрибут может применяться к методам и свойствам и приводит к флаг создаваемого члена как «abstract» и класс как абстрактный класс генератора.
...
Работает с исполняющей средой Objective-C посредством «false» отвечает на запрос Objective-C времени выполнения для определения, если указанный селектор реализована в этом классе.
...
Некоторые протоколы обычно используются как классы модели, те должны быть привязаны с помощью атрибута модели.
...
Начиная с MonoTouch 7.0 новых и усовершенствованных функций привязки протокол был включен.  Все определения, содержащего `[Protocol]` атрибут на самом деле создаст три вспомогательных классов, которые значительно улучшают способ использования протоколов:
...
Вы можете декорировать методов (на возвращаемое значение), параметры и свойства с `BindAs`. Единственное ограничение состоит в том элементов **не должны** находиться внутри `[Protocol]` или `[Model]` интерфейса.
...
При использовании для метода или свойства атрибута Bind действует для создания метода, который вызывает указанный селектор. Но не помечено полученный созданный метод `[Export]` атрибута, это означает, что он не может участвовать в переопределении метода. Обычно используется в сочетании с `Target` атрибут для реализации методов расширения Objective-C.
...
При использовании в метод получения или задания, `Bind` атрибут используется для изменения значения по умолчанию, чтобы вывести генератором кода при создании имен селектора Objective-C Get и set для свойства. По умолчанию при пометке свойство с именем «fooBar», генератор создал бы Экспорт «fooBar» для метода получения значения и «setFooBar:» для задания свойства. В некоторых случаях Objective-C не соответствуют этому соглашению, обычно они изменяют имени метода считывания, как «isFooBar».
...
Следующие создаст этот асинхронный метод, где `FileLoading` содержит свойства для доступа к «файлы» и «byteCount»:
...
Это происходит в случаях, где свойство объявлено в Objective-C, чтобы быть свойством «сохранить» или «назначение» вместо свойства «Копировать». Они обычно происходят в библиотеках сторонних разработчиков, которые неправильно «оптимизированные» разработчиками. В общем случае «сохранить» или «назначение» `NSString` с момента неверно определены свойства, `NSMutableString` или классы, производные от пользователя из `NSString` может привести к изменению содержимого строк независимо от кода библиотеки, слегка прерывание приложения. Обычно это происходит из-за преждевременное оптимизации.
...
В некоторых случаях нет константу, не связанные со значением, передаваемым в словаре.  Apple иногда использует константы открытых символов и иногда строковых констант.  По умолчанию `[Export]` атрибута в ваш предоставленный `EventArgs` класс будет использовать указанное имя в качестве открытых символов поиск во время выполнения.  Если это не так, а вместо этого он должен выполняться как строковая константа, а затем передать `ArgumentSemantic.Assign` значение для атрибута экспорта.

**Новые возможности Xamarin.iOS 8.4**

В некоторых случаях уведомления начнется жизни без аргументов, поэтому использование `[Notification]` без аргументов является приемлемым.  Однако в некоторых случаях будут вводиться параметры уведомления.  Для поддержки этого сценария, атрибут может применяться несколько раз.
...
При применении этого параметра в подписи метода указывает, что указанный параметр может иметь значение null, и что проверка должна быть выполнена для передачи значения null.

Если этот атрибут не имеет ссылочный тип, средство привязки создаст проверяет наличие значения, присваиваемого перед его передачей Objective-C и создаст убедитесь, что вызовет `ArgumentNullException` Если значение равно null.
...
Если значение «doAdd» равно true, то добавляется параметр `__mt_{0}_var List<NSObject>;`. Где `{0}` заменяется данного `listName`. В разделяемом классе дополнительный API-интерфейса, необходимо объявить этот резервное поле.
...
Обратите внимание, что аргумент LinkTarget неявно определяется Xamarin.iOS и не требуется задать.
...
При использовании копирования ноль для строк, эффективно генератор использует ту же строку на C# в качестве строки, потребляемой Objective-C без дополнительных создания нового `NSString` объекта и как избежать копирования данных из строки C# Objective-C-строку. Единственным недостатком использования копирования ноль строк является, необходимо убедиться, что любое строковое свойство, можно переносить, происходит было отмечено как «сохранить» или «копировать» имеет `DisableZeroCopy` набором атрибутов. Это нужно, так как дескриптор для копирования нулевой строки размещаются в стеке и является недопустимым после возврата функции.
