...
description: 'В этом документе описываются различные разделы, относящиеся к разработке Xamarin.iOS и Xamarin.Mac: code совместный доступ к API единой, привязка Objective-C библиотек, собственных ссылок, собственные типы и многое другое.'
ms.prod: xamarin
