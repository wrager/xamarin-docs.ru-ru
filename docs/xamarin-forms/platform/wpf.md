...
8. Изменить **MainWindow.xaml** файла проекта WPF. В `Window` , добавьте объявление пространства имен XML для **Xamarin.Forms.Platform.WPF** сборку и пространство имен:
    ...
    Теперь измените `Window` тег `wpf:FormsApplcationPage`. Изменение `Title` на имя приложения, например, **BoxViewClock**. Полный файл XAML должен выглядеть следующим образом:
