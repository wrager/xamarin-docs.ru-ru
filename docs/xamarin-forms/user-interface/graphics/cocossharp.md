...
**Развивать 2016: Cocos # в Xamarin.Forms**
...
В этом руководстве предполагается Xamarin.Forms проект с iOS, Android и .NET Standard проекта библиотеки.
Весь код будет записан в проекте .NET стандартные библиотеки; Тем не менее библиотеки должны быть добавлены проекты iOS и Android.
...
Для этого щелкните правой кнопкой мыши <span class="UIItem">пакетов</span> папки в проект .NET стандартные библиотеки и выберите <span class="UIItem">Добавление пакетов... </span>. Введите условие поиска <span class="UIItem">CocosSharp.Forms</span>выберите <span class="UIItem">CocosSharp для Xamarin.Forms</span>, нажмите кнопку <span class="UIItem">добавить пакет</span>.
...
Изначально `GameScene` класса будет практически пустым, — мы создадим только его соответствие ссылка в `HomePage`. Добавьте новый класс в проект .NET Standard библиотеки с именем `GameScene`. Он должен наследовать от `CCScene` следующим образом:
