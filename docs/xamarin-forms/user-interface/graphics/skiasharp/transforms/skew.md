...
description: В этой статье объясняется, как создать наклона преобразования наклона графических объектов в SkiaSharp и это демонстрируется с примерами кода.
ms.prod: xamarin
...
Если `xSkew` является отрицательной `ySkew`, результатом является поворот, но также масштабированный немного как UWP дисплее отображается.
