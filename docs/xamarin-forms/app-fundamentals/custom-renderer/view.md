...
> [!NOTE]
> **Примечание**: очень важно остановить и удаления объектов, которые обеспечивают доступ к камере в приложении Windows Phone или UWP. Невыполнение этого требования может повлиять на другие приложения, которые пытаются получить доступ к камере устройства. Дополнительные сведения см. в разделе и [краткое руководство: запись видео с помощью MediaCapture API](https://msdn.microsoft.com/library/windows/apps/xaml/dn642092.aspx) для приложений среды выполнения Windows и [камеры предварительного просмотра](https://msdn.microsoft.com/windows/uwp/audio-video-camera/simple-camera-preview-access) для приложений UWP.
