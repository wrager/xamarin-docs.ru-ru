...
Этот класс определяет `CustomPin` как наследующий свойства [ `Pin` ](https://developer.xamarin.com/api/type/Xamarin.Forms.Maps.Pin/) класса и добавления `Id`, и `Url` свойства.
...
> [!NOTE]
> **Примечание**: это необязательно для предоставления пользовательского средства отрисовки в каждом проекте платформы. Если пользовательское средство отрисовки не зарегистрирован, будет использоваться модуль подготовки отчетов по умолчанию для базового класса элемента управления.
