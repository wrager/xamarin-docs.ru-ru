...
description: Выберите фотографию из библиотеки рисунков телефона с помощью помощью DependencyService
ms.prod: xamarin
...
- **[Реализация Windows Phone](#Windows_Phone_Implementation)**  &ndash; Узнайте, как реализовать интерфейс в машинном коде для Windows Phone 8.1.
...
Реализация iOS `IPicturePicker` интерфейс использует [ `UIImagePickerController` ](https://developer.xamarin.com/api/type/UIKit.UIImagePickerController/) как описано в [ **выберите фотографию из коллекции** ](https://developer.xamarin.com/recipes/ios/media/video_and_photos/choose_a_photo_from_the_gallery/) инструкций и [пример кода](https://github.com/xamarin/recipes/tree/master/ios/media/video_and_photos/choose_a_photo_from_the_gallery).
...
## <a name="windows-phone-81-implementation"></a>Реализация Windows Phone 8.1

Реализация Windows Phone 8.1 аналогична реализации UWP, за исключением одного Существенная разница, окажет значительное влияние: `PickSingleFileAsync` метод `FileOpenPicker` недоступна. Вместо этого `GetImageStreamAsync` необходимо вызвать метод `PickSingleFileAndContinue`. Этот метод возвращает библиотека фотографии отображается для пользователя, когда выбор пользователя к фотографии получает сигнал посредством вызова `OnActivated` метод `App` класса.

По этой причине [ `App` ](https://github.com/xamarin/xamarin-forms-samples/blob/master/DependencyService/DependencyServiceSample/WinPhone/App.xaml.cs) класс, созданный с помощью шаблона проекта Xamarin.Forms в проекте Windows Phone 8.1 были дополнены свойство типа `TaskCompletionSource` и переопределение `OnActivated` метод:
...
`OnActivated` Метод может вызываться по нескольким причинам, включая запуск приложения. Код ограничится только такие вызовы после выбора открытия файла завершена и затем получает `Stream` объекта из `StorageFile`.

[ `PicturePickerImplementation` ](https://github.com/xamarin/xamarin-forms-samples/blob/master/DependencyService/DependencyServiceSample/WinPhone/PicturePickerImplementation.cs) Класс содержит `GetImageStreamAsync` метод, который создает `FileOpenPicker` и вызывает метод `PickSingleFileAndContainue`:
