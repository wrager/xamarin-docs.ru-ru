...
description: В этой статье показано, как использовать класс NavigationPage выполнить переход в стек последним поступил — первым обслужен (LIFO) страниц.
ms.prod: xamarin
...
- На универсальной платформе Windows панели навигации в верхней части страницы, на которой отображается заголовок отсутствует.
