...
Проверка также автоматически запускается при каждом изменении связанного свойства. Например, когда двустороннюю привязку в `LoginView` задает `UserName` или `Password` запускается свойства проверки. В следующем примере кода показано, как это происходит:
