...
> [!NOTE]
> Если запросу выполняется к таблице с ожидающими локального обновления, опрашивающего сначала выполнить push в контекст синхронизации. Это сводит к минимуму конфликтов между изменениями, которые уже поставлены в очередь и новые данные из экземпляра мобильных приложений Azure.
...
> [!NOTE]
> Вызов `PurgeAsync` с `force` равным `true` будут потеряны все ожидающие изменения.
