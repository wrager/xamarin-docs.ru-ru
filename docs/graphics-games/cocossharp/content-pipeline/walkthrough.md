...
description: "Средство MonoGame конвейер служит для создания и управления MonoGame проектов. Файлы проектов обрабатываются с помощью средства Monogame конвейера и выводимые в виде .xnb файлов для использования в приложениях CocosSharp и MonoGame."
ms.topic: article
ms.prod: xamarin
...
_Средство MonoGame конвейер служит для создания и управления MonoGame проектов. Файлы проектов обрабатываются с помощью средства Monogame конвейера и выводимые в виде .xnb файлов для использования в приложениях CocosSharp и MonoGame._

Средство MonoGame конвейера предоставляет среду для использования для преобразования содержимого файлов в **.xnb** файлов для использования в приложениях CocosSharp и MonoGame. Сведения о содержимого конвейеров и почему они полезны при разработке игры см [этот вводные сведения о конвейерах содержимого](~/graphics-games/cocossharp/content-pipeline/introduction.md)
...
# <a name="monogame-pipeline-platform-discussion"></a>Обсуждение платформы MonoGame конвейера

Конвейер MonoGame средство доступно в Windows, Linux и OS X. В этом пошаговом руководстве будет запустите средство на Windows, но он может располагаться после Mac и Linux также. Сведения о получении средства на Max или Linux см. в разделе [эту страницу](http://www.monogame.net/2015/01/09/monogame-pipeline-tool-available-for-macos-and-linux/).
...
После загрузки, мы запуск с помощью программы установки и примите значения по умолчанию. По завершении работы установщика средство MonoGame конвейера устанавливается и можно найти в меню Пуск поиска:

![](walkthrough-images/image3.png "По завершении работы установщика устанавливается средство MonoGame конвейера и можно найти в меню Пуск поиска")

Запустите средство MonoGame конвейера:

![](walkthrough-images/image4.png "Запустите средство MonoGame конвейера")

После запуска средства MonoGame конвейера, мы сможем сделать наш проекты игры и содержимого.


# <a name="creating-an-empty-cocossharp-project"></a>Создавая пустой проект CocosSharp

Следующим шагом является создание проекта CocosSharp. Очень важно, что мы CocosSharp сначала создать проект, чтобы наши содержимого проекта можно сохранить в структуре папки, созданные в проекте CocosSharp. Сведения о создании нового проекта см. в разделе [руководство BouncingGame](~/graphics-games/cocossharp/first-game/part1.md). В этом руководстве будут созданы проект с именем BouncingGame, но подойдет любой существующий проект CocosSharp. При наличии существующего проекта CocosSharp, который вы хотите добавить содержимое, вы можете использовать этот проект вместо BouncingGame проекта.
...
После создания проекта средство MonoGame конвейера выведет информацию о проекте при корневой **ContentProject** выбран элемент:

![](walkthrough-images/image9.png "После создания проекта средство MonoGame конвейера выдает информацию о проекте, при выборе ContentProject корневой элемент")

Рассмотрим некоторые из наиболее важных параметров для содержимого проекта.


## <a name="output-folder"></a>Папка выходных данных
...
# <a name="processing-files-in-the-monogame-pipelinetool"></a>Обработка файлов в MonoGame PipelineTool
...
Мы добавим в свой проект два файла:
 ...
 - Объект **.spritefont** файл, который будет использоваться для рисования текста на экране. Средство ContentPipeline поддерживает создание новых файлов .spritefont, поэтому нет файла для загрузки.
...
Мы не будем рассматривать приведенный выше код так, как рассматривается работа с CocosSharp визуальных объектов, таких как CCSprite и CCLabelTtf [CocosSharp начальный руководства](~/graphics-games/cocossharp/first-game/index.md).
