...
description: В этом документе описывается PhotoKit, обсуждение модели объектов, как для запроса модели данных и сохранение изменений в библиотеке фото.
ms.prod: xamarin
...
PhotoKit представляет этих средств, в то, что он вызывает объекты модели. Модель объектов, представляющих фотографии и видео, сами имеют тип `PHAsset`. Объект `PHAsset` содержит метаданные, такие как тип актива мультимедиа и дата его создания.
...
PhotoKit позволяет легко запрашивать данные модели через различные методы выборки. Например, чтобы получить все образы, следует вызвать `PFAsset.Fetch`, передав `PHAssetMediaType.Image` тип носителя.
