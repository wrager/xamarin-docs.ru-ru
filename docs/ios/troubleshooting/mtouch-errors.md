...
### <a name="a-namemt0116mt0116-invalid-architecture-arch-32-bit-architectures-are-not-supported-when-deployment-target-is-11-or-later-make-sure-the-project-does-not-build-for-a-32-bit-architecture"></a><a name="MT0116"/>MT0116: Недопустимая архитектура: {arch}. 32-разрядных архитектур не поддерживаются при цель развертывания 11 или более поздней версии. Убедитесь, что проект не построен для 32-разрядной архитектуры.
...
### <a name="a-namemt0117mt0117-cant-launch-a-32-bit-app-on-a-simulator-that-only-supports-64-bit"></a><a name="MT0117"/>MT0117: Не удается запустить 32-разрядное приложение в имитаторе, который поддерживает только 64-разрядной.

### <a name="a-namemt0118mt0118-aot-files-could-not-be-found-at-the-expected-directory-msymdir"></a><a name="MT0118"/>MT0118: Aot файлов не удалось найти в предполагаемом каталоге {msymdir}.

<!-- 0119 - 0123: free to use -->

### <a name="a-namemt0123mt0123-the-executable-assembly--does-not-reference-"></a><a name="MT0123"/>MT0123: Исполняемая сборка * не ссылается на *.
...
### <a name="a-namemt0124mt0124-could-not-set-the-current-language-to-lang-according-to-langlang-exception"></a><a name="MT0124"/>MT0124: Не удалось задать текущего языка {lang} (согласно LANG = {LANG}): {исключение}
...
### <a name="a-namemt0128mt0128-could-not-touch-the-file--"></a><a name="MT0128"/>MT0128: Не удалось касание файла "*": *
...
### <a name="a-namemt1111mt1111-application-launched-successfully-but-its-not-possible-to-wait-for-the-app-to-exit-as-requested-because-its-not-possible-to-detect-app-termination-when-launching-using-gdbserver"></a><a name="MT1111"/>MT1111: Приложение успешно запущен, но невозможно дождаться выхода по запросу, так как не удается обнаружить завершение приложения при запуске программы с помощью gdbserver приложению.
...
### <a name="a-namemt2090mt2090-inliner-failed-processing-"></a><a name="MT2090"/>MT2090: Встраивания ошибка при обработке `...`.
...
### <a name="a-namemt2100mt2100-smart-enum-conversion-preserver-failed-processing-"></a><a name="MT2100"/>MT2100: Смарт-Сохранить преобразование перечисления не удалось обработки `...`.
...
### <a name="a-namemt2103mt2103-error-processing-assembly--"></a><a name="MT2103"/>MT2103: Ошибка при обработке сборки "\*": *
...
### <a name="a-namemt2104mm2104-unable-to-link-assembly-0-as-it-is-mixed-mode"></a><a name="MT2104"/>MM2104: Не удалось связать сборки «{0}», как и смешанном режиме.

Сборки смешанного режима не могут обрабатываться компоновщиком.

Https://msdn.microsoft.com/en-us/library/x0w2664k.aspx для получения дополнительной информации см. на сборки смешанного режима.
...
### <a name="a-namemt4169mt4169-failed-to-generate-a-pinvoke-wrapper-for-method-message"></a><a name="MT4169"/>MT4169: Не удалось создать оболочку P/Invoke для {метода}: {message}
...
### <a name="a-namemt4170mt4170-the-registrar-cant-convert-from-managed-type-to-native-type-for-the-return-value-in-the-method-method"></a><a name="MT4170"/>MT4170: Регистратора невозможно преобразовать из «{управляемый тип}» в «{собственный тип}» для возвращаемого значения в метод {}.

См. в описании ошибки <a href="#MT4172">MT4172</a>.

### <a name="a-namemt4171mt4171-the-bindas-attribute-on-the-member-member-is-invalid-the-bindas-type-type-is-different-from-the-property-type-type"></a><a name="MT4171"/>MT4171: Недопустимый атрибут BindAs на член {}: {type} BindAs тип отличается от типа свойства {type}.

Убедитесь, что типа в атрибуте BindAs соответствует тип члена, которому он присоединен.

### <a name="a-namemt4172mt4172-the-registrar-cant-convert-from-native-type-to-managed-type-for-the-parameter-parameter-name-in-the-method-method"></a><a name="MT4172"/>MT4172: Регистратора невозможно преобразовать из «{собственный тип}» в «{управляемый тип}» для параметра «{имени}» в методе {метод}.
...
### <a name="a-namemt8024mt8024-could-not-find-a-valid-extension-type-for-the-smart-enum-smarttype-please-file-a-bug-at-httpsbugzillaxamarincom"></a><a name="MT8024"/>MT8024: Не удалось найти является действительным типом расширения смарт-перечисление {smart_type}. Можно зарегистрировать ошибку на https://bugzilla.xamarin.com.
