---
title: Представления стека в Xamarin.iOS
...
---
# <a name="stack-views-in-xamarinios"></a>Представления стека в Xamarin.iOS
...
> [!IMPORTANT]
> `UIStackView` разработана как представление контейнера без отрисовки и таким образом, он не рисуется на холст, как и другие подклассы `UIView`. Поэтому такие как при задании свойства `BackgroundColor` или переопределение `DrawRect` не будет действовать visual.
