...
description: Этот документ описывает framework MapKit операций ввода-вывода и как она используется с Xamarin.iOS. В этом примере рассматривается добавление карты, стиль, панорамирование и масштаб, работать с местонахождения пользователя, добавление заметок, работать с выносками и наложения и выполнения локального поиска.
ms.prod: xamarin
