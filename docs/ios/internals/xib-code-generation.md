...
description: В этом документе описывается, как Xamarin.iOS создает код для сопоставления файлов .xib в C#, программно доступности визуальные элементы управления.
ms.prod: xamarin
...
Для любого  **{0}.xib** файл с действием построения *страницы*, если  **{0}. xib.designer.cs** файл также существует в проекте Visual Studio для Mac Разделяемые классы создаст в этот файл конструктора для всех пользовательских классов его можно найти в **.xib** файл со свойствами для выпусков и частичных методов для всех действий. Создание кода просто обеспечивается наличие этого файла.
