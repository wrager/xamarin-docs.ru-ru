---
title: "Безопасность транспорта приложения"
...
> [!IMPORTANT]
> **Примечание:** безопасность транспорта приложения не применяется к с помощью приложений Xamarin **HTTPClient управляемых реализаций**. Он применяется для соединений с помощью CFNetwork **реализации HTTPClient** или **NSURLSession HTTPClient реализации** только.
...
> Безопасность транспорта приложения заблокировал нагрузки на ресурсы открытый текст HTTP (http://www.-the-blocked-domain.com), так как он не защищен. Временные исключения можно настроить через файл Info.plist приложения.
...
Также предоставляет Apple [TLSTool пример приложения](https://developer.apple.com/library/mac/samplecode/sc1236/Introduction/Intro.html#//apple_ref/doc/uid/DTS40014927-Intro-DontLinkElementID_2) , можно скомпилировать (или при необходимости перекодировать Xamarin и C#) и используется для диагностики проблем ATS/TLS. См. в разделе [Opting масштабированием ATS](#Opting-Out_of_ATS) информацию о решении этой проблемы.
...
[ ![](ats-images/ats01.png "В представлении исходный файл Info.plist")](ats-images/ats01.png)
...
[ ![](ats-images/ats02.png "В представлении исходный файл Info.plist")](ats-images/ats02.png)

> [!IMPORTANT]
> **Примечание:** Если приложению требуется подключение небезопасных веб-сайт, необходимо выполнить **всегда** введите домен, как исключение с помощью `NSExceptionDomains` вместо отключения ATS полностью с помощью `NSAllowsArbitraryLoads`. `NSAllowsArbitraryLoads` следует использовать только в чрезвычайных ситуациях аварийного.
