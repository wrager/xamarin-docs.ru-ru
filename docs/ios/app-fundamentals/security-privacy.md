...
> [!IMPORTANT]
> В примере заданному выше, не удалось задать `NSHomeKitUsageDescription` ключа в `Info.plist` файла приведет к появлению приложения _без вмешательства пользователя со сбоем_ (закрывается в системе во время выполнения) без ошибок, при выполнении в iOS 10 (или более поздней).
