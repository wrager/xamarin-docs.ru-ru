...
> [!IMPORTANT]
> Хотя и существует возможность назначить действий, таких как `TouchUpInside` для `UIButton` в конструктор при создании iOS **обработчик событий**, он никогда не вызываться, так как для Apple TV нет сенсорном экране или поддерживает событий сенсорного экрана. Следует всегда использовать значение по умолчанию **тип действия** при создании **действия** для tvOS элементов пользовательского интерфейса.
...
> [!IMPORTANT]
> Хотя и существует возможность назначить действий, таких как `TouchUpInside` для `UIButton`, он никогда не вызываться, так как для Apple TV нет сенсорном экране или поддерживает событий сенсорного экрана. Следует всегда использовать события например **AllEvents** или **PrimaryActionTriggered**.
